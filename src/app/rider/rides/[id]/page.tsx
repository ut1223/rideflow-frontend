...
import { RatingForm } from "@/features/rider/RatingForm";
import { PaymentPanel } from "@/features/rider/PaymentPanel";
...
export default function RiderRideDetailPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Ride Details"
        description={`Requested ${formatDateTime(ride.requestedAt)}`}
        action={
          canCancel ? (
      ...
      {ride.status === "COMPLETED" && (
        <Card className="mt-4">
          <CardHeader>
            <CardTitle>Rate this ride</CardTitle>
          </CardHeader>
          <CardContent>
            <RatingForm rideId={ride.id} rating={ride.rating} />
          </CardContent>
        </Card>
      )}
